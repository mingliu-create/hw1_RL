...
### 2. 策略顯示與價值評估 (HW1-2 - Policy Evaluation)
- **隨機策略**: 每格初始化即生成隨機箭頭 $\pi(s)$。
- **價值計算**: 實現了 $V(s) = R + \gamma V(s')$ 疊代算法。
- **黃金路徑高亮**: 透過 JavaScript 實時渲染，$V(s)$ 最高的動作會觸發金色發光特效。

### 3. 最佳政策推導與價值迭代 (HW1-3 - Value Iteration)
- **價值迭代演算法**: 實作 Value Iteration 演算法疊代求出最佳價值函數 $V^*(s)$ 與對應的最佳政策 $\pi^*(s)$。
- **最佳政策顯示**: 取代原本的隨機行動，在介面上顯示由算法推導出的每個狀態的最佳動作（箭頭）。
- **最佳價值函數**: 更新所有狀態的數值標籤，顯示在最佳政策下的期望回報。
